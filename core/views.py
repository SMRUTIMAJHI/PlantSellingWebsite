...
# In your views.py (or wherever you're trying to import PaymentStatus)
...
def handlerequest(request):
   if request.method == "POST":
        try:
            ...
            logger.debug(f"Payment ID: {payment_id}, Order ID: {order_id}, Signature: {signature}")
            ...
            order_db.save()
            logger.debug("Order updated with payment details.")
            ...
            try:
                ...
                # Capture payment
                amount = order_db.get_total_price() * 100  # convert to paisa
                ...
                if payment_status:
                    ...
                    # Set session message for successful order
                    request.session["order_complete"] = "Your order is successfully placed. You will receive your order within 5-7 working days."
                    ...
                    order_db.save()
                    request.session["order_failed"] = "Unfortunately, your order could not be placed. Please try again."
                    return redirect("/")
            ...
            logger.exception("An error occurred during payment handling.")
